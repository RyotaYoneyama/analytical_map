...
- FP、FNを5つに分類でき、検出の失敗を分析できる。
- COCOのフォーマットに準ずることで、新たなデータ構造の理解がいらない。
## 背景
ロボティクスに使用可能なカメラは多種多様である。選択肢の数だけカメラ制御を行うためのSDKが提供されている。
しかし、インターフェースは統一されておらず、カメラ毎の実装となっているのが現状である。  
一部のカメラではROSに対応しているものもあるが、所望のカメラ制御を行いたい場合は、内部処理に手を加える必然性は残るため、汎用的に使えるとは言い難い。  
これらの課題を解決するため、下記内容を含む統合カメラ制御ソフトウェアとして ym_camera を作成した。
* カメラ制御インターフェースの共通化
* 機能拡張性の実現
* カメラ制御周辺機能の統合
## ym_camera基本方針  
* カメラ制御インターフェースを継承する形で、各メーカーのSDK・ライブラリを利用したカメラクラスを実装する。  
* 画像取得やカメラ設定変更だけでなく、TOFカメラやステレオカメラのようなDepthカメラへの拡張も想定する。  
* カメラクラスの利用方法を示すためのデモアプリケーションもリポジトリに含める。
* Windows (Visual Studio)、Linux、ROSへの対応を考慮する。
* doxygenを利用してドキュメント作成を省力化する。
# 参考資料
